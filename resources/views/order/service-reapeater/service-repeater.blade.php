...
@push('inner-script')
    ...
    <script>
        ...
        $(document).on('change', '.service_id', function($event) {
            ...
            var viewIcon = $($($(this).parents('.form-group')[0]).find('.view-icon')[0]);
            ...
            if (id != null && id != "") {
                ...
                if (service != undefined && service != null) {
                    ...
                    if(service['image'] != null && service['image'].length > 0){
                        ...
                        viewIcon.attr('data-image', "")
                        viewIcon.removeClass('d-block')
                    }
                        ...
                        measurementinputs += `
                            <tr class="">
                                ...
                                <td>
                                    ...
                                    <input type="hidden" name="services[` + serviceIndex + `][measurements][` + index +
                            `][id]" value="` + value['id'] + `" />
                                    <input type="hidden" name="services[` + serviceIndex + `][measurements][` + index +
                    ...
                    $.each(designs, function(index, value) {
                        ...
                        designInputs += `
                            <tr class="">
                                <td class='w-5'><small >` + value['name'] + `</small></td>
                                <td>
                                    <input type="hidden" value="` + value['id'] + `" name="services[` + serviceIndex +
                            `][designs][` + index + `][id]"/>
                                    <select class="form-control form-control-sm" name="services[` + serviceIndex +
                            `][designs][` + index + `][style_id]">` +
                            options +
                            `</select>
                                </td>
                            </tr>
        ...
        $(document).on('click', '.view-icon', function() {
            ...
            var imageUrl = "{{ URL::to('/') }}/" + $(this).attr('data-image');
            $($('#preview-modal').find('img')[0]).attr('src', imageUrl)
            $('#preview-modal').modal('show')
        });


        $('#preview-modal').on('hidden.bs.modal', function() {
            $($('#preview-modal').find('img')[0]).attr('src', "")
        })
    </script>
@endpush
